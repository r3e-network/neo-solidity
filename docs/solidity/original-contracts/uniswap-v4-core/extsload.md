...
- Primary issue: overloaded function 'extsload' with 1 parameter(s) is not supported; Neo ABI dispatches by name and argument count only, so overloads that differ only in parameter types cannot be distinguished at runtime
...
| --- | --- | --- |
| error | VALIDATION_ERROR | overloaded function 'extsload' with 1 parameter(s) is not supported; Neo ABI dispatches by name and argument count only, so overloads that differ only in parameter types cannot be distinguished at runtime |
