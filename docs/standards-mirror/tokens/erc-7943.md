...
real-world assets. Where [ERC-3643](./erc-3643) (T-REX) is fungible-only and shipped
as a heavy framework, uRWA is the umbrella interface that any RWA
token — fungible ([ERC-20](./erc-20)), non-fungible ([ERC-721](./erc-721)), semi-fungible
([ERC-3525](./erc-3525)), or multi-token ([ERC-1155](./erc-1155)) — can declare. Its surface is
...
operations are the same set [ERC-3643](./erc-3643) ships, made available across
token types.
...
| `isTransferCompliant(...)` | `IsTransferCompliant(...)` returning `(bool, string)` | Wrapper around [ERC-7144](./erc-7144) validator + freeze list |
...
- **[ERC-7144](./erc-7144)** — transfer validation hook. uRWA's
  `isTransferCompliant` is naturally backed by an [ERC-7144](./erc-7144) validator.
...
For existing T-REX ([ERC-3643](./erc-3643)) deployments adding uRWA:
...
uRWA + [ERC-7144](./erc-7144) validator combination is lighter and expresses the same
...
fungible RWAs (T-REX-style stablecoins), bond NFTs ([ERC-3475](/standards-mirror/defi/erc-3475) / 7092
