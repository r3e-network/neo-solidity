...
ERC-777 was an attempt to fix [ERC-20](./erc-20)'s biggest ergonomics issue — tokens sent to a
contract that wasn't expecting them get stuck. It adds `tokensReceived` and
`tokensToSend` hooks, registered via the [ERC-1820](/standards-mirror/infrastructure/erc-1820) registry, plus an operator model
...
Checked-in snapshot: Solidity 1 / 3 assertions pass; Neo C# 3 / 3 assertions pass. This is a validation snapshot, not an all-green parity certification; see [TestNet Results](/standards-mirror/deployments/RESULTS) for failure details.

Source pairs: [`docs/standards-mirror/deployments/erc-777/`](https://github.com/r3e-network/neo-devpack-solidity/tree/main/docs/standards-mirror/deployments/erc-777).
:::
