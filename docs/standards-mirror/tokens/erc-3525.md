...
[ERC-20](./erc-20)-like `value` quantity within each unit. Use cases: bonds (each tokenId is an
...
Checked-in snapshot: Solidity 4 / 4 assertions pass; Neo C# 3 / 3 assertions pass. This is a validation snapshot, not an all-green parity certification; see [TestNet Results](/standards-mirror/deployments/RESULTS) for failure details.

Source pairs: [`docs/standards-mirror/deployments/erc-3525/`](https://github.com/r3e-network/neo-devpack-solidity/tree/main/docs/standards-mirror/deployments/erc-3525).
:::
...
[DisplayName("Bond")]
[ContractPermission("*", "*")]
[SupportedStandards(NepStandard.Nep11)]      // also exposes ERC-3525 semantics via dedicated methods
