...
`setInterfaceImplementer(account, interfaceHash, implementer)`. [ERC-777](/standards-mirror/tokens/erc-777)'s
`tokensReceived` hook discovery uses this.

### Why It's Needed on Ethereum

[ERC-165](./erc-165) only works for contracts that **implement** the standard themselves. ERC-1820
...
Source pairs: [`docs/standards-mirror/deployments/erc-1820/`](https://github.com/r3e-network/neo-devpack-solidity/tree/main/docs/standards-mirror/deployments/erc-1820).
:::
...
The big ERC-1820 use case — delegating [ERC-777](/standards-mirror/tokens/erc-777) token reception to another contract
