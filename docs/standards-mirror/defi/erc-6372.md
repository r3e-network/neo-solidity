...
A meta-standard that pairs with [ERC-5805](./erc-5805). Every governance contract advertises its
...
Source pairs: [`docs/standards-mirror/deployments/erc-6372/`](https://github.com/r3e-network/neo-devpack-solidity/tree/main/docs/standards-mirror/deployments/erc-6372).
:::
...
{
    // EIP-6372 `mode=timestamp` is unix seconds — Runtime.Time is unix ms,
    // so divide by 1000 to match the spec's default unit.
    public static ulong  Clock()      => Runtime.Time / 1000;
    [Safe]
...
milliseconds since unix epoch (the timestamp clock divides by 1000 so its value
matches the seconds-precision unit that EIP-6372 governance frontends assume).
Both are deterministic per-block primitives.
