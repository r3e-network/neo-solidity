...
ERC-2612 patches a structural problem with [ERC-20](/standards-mirror/tokens/erc-20): every approval costs a separate
transaction, which means users must spend gas before they can use a DeFi protocol.
Permit lets users sign an [EIP-712](./eip-712) typed message off-chain; the protocol submits it
...
Phishing replay (sign infinite permit), signature malleability (pre-[EIP-2098](/standards-mirror/protocol-eips/eip-2098)),
