...
ERC-6551 gives every ERC-721 NFT a deterministic, smart-contract wallet — a
...
2. **Account implementation** (`IERC6551Account`) — a minimal proxy clone
   (often using ERC-1167) of an account contract. The account exposes:
...
Combined with **ERC-1271** (smart-contract signatures) the TBA can sign
...
- **Smart-contract signers** are native (via NEP-30 `Verify`) — no ERC-1271
  shim required.
...
The minimal proxy (ERC-1167) and the singleton registry (ERC-6551 spec)
