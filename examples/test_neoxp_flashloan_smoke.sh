#!/usr/bin/env bash
# Smoke test: FlashLoan — compile, deploy, invoke on Neo Express.
#
# Verifies:
#   - Compilation to NEF + manifest
#   - Deployment on neo-express
#   - FLASH_LOAN_FEE() returns 9
#   - totalDeposits() starts at 0
#   - poolBalance() returns 0
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/neo-sol-flashloan.XXXXXX")"
...
# Compile
"$NEO_SOLC_BIN" "$ROOT_DIR/examples/famous/FlashLoan.sol" -I "$ROOT_DIR/devpack" -O2 -o FlashLoan >/dev/null
...
CONTRACT_HASH="$(run_neoxp contract hash -i "$CHAIN" FlashLoan.nef node1 | tr -d '\r')"
echo "(info) Contract hash: $CONTRACT_HASH"
run_neoxp contract deploy -i "$CHAIN" FlashLoan.nef node1 -j >/dev/null

# Test: FLASH_LOAN_FEE() == 9
cat > invoke.json <<JSON
{ "contract": "$CONTRACT_HASH", "operation": "FLASH_LOAN_FEE", "args": [] }
JSON
OUT="$(run_neoxp contract invoke -r -j -i "$CHAIN" invoke.json node1)"
if [ "$(echo "$OUT" | jq -r '.state')" != "HALT" ]; then
  echo "error: FLASH_LOAN_FEE() did not HALT"; echo "$OUT"; exit 1
fi
VAL="$(echo "$OUT" | jq -r '.stack[0].value')"
if [ "$VAL" != "9" ]; then echo "error: FLASH_LOAN_FEE() expected 9, got '$VAL'"; exit 1; fi
echo "(ok) FLASH_LOAN_FEE() = 9"

# Test: totalDeposits() == 0
cat > invoke.json <<JSON
{ "contract": "$CONTRACT_HASH", "operation": "totalDeposits", "args": [] }
JSON
OUT="$(run_neoxp contract invoke -r -j -i "$CHAIN" invoke.json node1)"
if [ "$(echo "$OUT" | jq -r '.state')" != "HALT" ]; then
  echo "error: totalDeposits() did not HALT"; echo "$OUT"; exit 1
fi
VAL="$(echo "$OUT" | jq -r '.stack[0].value')"
if [ "$VAL" != "0" ]; then echo "error: totalDeposits() expected 0, got '$VAL'"; exit 1; fi
echo "(ok) totalDeposits() = 0"

# Test: poolBalance() == 0
cat > invoke.json <<JSON
{ "contract": "$CONTRACT_HASH", "operation": "poolBalance", "args": [] }
JSON
OUT="$(run_neoxp contract invoke -r -j -i "$CHAIN" invoke.json node1)"
if [ "$(echo "$OUT" | jq -r '.state')" != "HALT" ]; then
  echo "error: poolBalance() did not HALT"; echo "$OUT"; exit 1
fi
VAL="$(echo "$OUT" | jq -r '.stack[0].value')"
if [ "$VAL" != "0" ]; then echo "error: poolBalance() expected 0, got '$VAL'"; exit 1; fi
echo "(ok) poolBalance() = 0"

echo "✅ FlashLoan smoke test passed"
