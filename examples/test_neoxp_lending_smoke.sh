#!/usr/bin/env bash
# Smoke test: SimpleLending — compile, deploy, invoke on Neo Express.
#
# Verifies:
#   - Compilation to NEF + manifest
#   - Deployment on neo-express
#   - COLLATERAL_RATIO() returns 150
#   - INTEREST_RATE_PER_BLOCK() returns 1
#   - totalDeposited() starts at 0
#   - totalBorrowed() starts at 0
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/neo-sol-lending.XXXXXX")"
...
# Compile
"$NEO_SOLC_BIN" "$ROOT_DIR/examples/famous/SimpleLending.sol" \
  -I "$ROOT_DIR/devpack" -O2 -o SimpleLending >/dev/null
...
CONTRACT_HASH="$(run_neoxp contract hash \
  -i "$CHAIN" SimpleLending.nef node1 | tr -d '\r')"
echo "(info) Contract hash: $CONTRACT_HASH"
run_neoxp contract deploy -i "$CHAIN" SimpleLending.nef node1 -j >/dev/null

# Test: COLLATERAL_RATIO() == 150
cat > invoke.json <<JSON
{ "contract": "$CONTRACT_HASH", "operation": "COLLATERAL_RATIO", "args": [] }
JSON
OUT="$(run_neoxp contract invoke -r -j -i "$CHAIN" invoke.json node1)"
if [ "$(echo "$OUT" | jq -r '.state')" != "HALT" ]; then
  echo "error: COLLATERAL_RATIO() did not HALT"; echo "$OUT"; exit 1
fi
VAL="$(echo "$OUT" | jq -r '.stack[0].value')"
if [ "$VAL" != "150" ]; then
  echo "error: COLLATERAL_RATIO() expected 150, got '$VAL'"; exit 1
fi
echo "(ok) COLLATERAL_RATIO() = 150"

# Test: totalDeposited() == 0
cat > invoke.json <<JSON
{ "contract": "$CONTRACT_HASH", "operation": "totalDeposited", "args": [] }
JSON
OUT="$(run_neoxp contract invoke -r -j -i "$CHAIN" invoke.json node1)"
if [ "$(echo "$OUT" | jq -r '.state')" != "HALT" ]; then
  echo "error: totalDeposited() did not HALT"; echo "$OUT"; exit 1
fi
VAL="$(echo "$OUT" | jq -r '.stack[0].value')"
if [ "$VAL" != "0" ]; then
  echo "error: totalDeposited() expected 0, got '$VAL'"; exit 1
fi
echo "(ok) totalDeposited() = 0"

# Test: totalBorrowed() == 0
cat > invoke.json <<JSON
{ "contract": "$CONTRACT_HASH", "operation": "totalBorrowed", "args": [] }
JSON
OUT="$(run_neoxp contract invoke -r -j -i "$CHAIN" invoke.json node1)"
if [ "$(echo "$OUT" | jq -r '.state')" != "HALT" ]; then
  echo "error: totalBorrowed() did not HALT"; echo "$OUT"; exit 1
fi
VAL="$(echo "$OUT" | jq -r '.stack[0].value')"
if [ "$VAL" != "0" ]; then
  echo "error: totalBorrowed() expected 0, got '$VAL'"; exit 1
fi
echo "(ok) totalBorrowed() = 0"

echo "✅ SimpleLending smoke test passed"
