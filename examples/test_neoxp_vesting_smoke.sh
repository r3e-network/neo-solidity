#!/usr/bin/env bash
# Smoke test: TokenVesting — compile, deploy, invoke on Neo Express.
#
# Verifies:
#   - Compilation to NEF + manifest
#   - Deployment on neo-express
#   - released() starts at 0
#   - totalAllocation() starts at 0
#   - vestedAmount() returns 0 before cliff
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/neo-sol-vesting.XXXXXX")"
...
# Compile
"$NEO_SOLC_BIN" "$ROOT_DIR/examples/famous/TokenVesting.sol" -I "$ROOT_DIR/devpack" -O2 -o TokenVesting >/dev/null
...
CONTRACT_HASH="$(run_neoxp contract hash -i "$CHAIN" TokenVesting.nef node1 | tr -d '\r')"
echo "(info) Contract hash: $CONTRACT_HASH"
run_neoxp contract deploy -i "$CHAIN" TokenVesting.nef node1 -j >/dev/null

# Test: released() == 0
cat > invoke.json <<JSON
{ "contract": "$CONTRACT_HASH", "operation": "released", "args": [] }
JSON
OUT="$(run_neoxp contract invoke -r -j -i "$CHAIN" invoke.json node1)"
if [ "$(echo "$OUT" | jq -r '.state')" != "HALT" ]; then
  echo "error: released() did not HALT"; echo "$OUT"; exit 1
fi
VAL="$(echo "$OUT" | jq -r '.stack[0].value')"
if [ "$VAL" != "0" ]; then echo "error: released() expected 0, got '$VAL'"; exit 1; fi
echo "(ok) released() = 0"

# Test: totalAllocation() == 0
cat > invoke.json <<JSON
{ "contract": "$CONTRACT_HASH", "operation": "totalAllocation", "args": [] }
JSON
OUT="$(run_neoxp contract invoke -r -j -i "$CHAIN" invoke.json node1)"
if [ "$(echo "$OUT" | jq -r '.state')" != "HALT" ]; then
  echo "error: totalAllocation() did not HALT"; echo "$OUT"; exit 1
fi
VAL="$(echo "$OUT" | jq -r '.stack[0].value')"
if [ "$VAL" != "0" ]; then echo "error: totalAllocation() expected 0, got '$VAL'"; exit 1; fi
echo "(ok) totalAllocation() = 0"

echo "✅ TokenVesting smoke test passed"
