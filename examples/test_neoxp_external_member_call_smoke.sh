#!/usr/bin/env bash
# End-to-end external member-call smoke test using bundled Neo-Express (neoxp).
#
# Verifies that contract/interface-style calls like:
#   `ICallee(target).foo(41)`
# are lowered into `System.Contract.Call` with correct syscall stack order.
#
# This exercises the non-low-level external call path (not `address.call(...)`).
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/neo-devpack-solidity-neoxp-extcall.XXXXXX")"
...
CALLEE_HASH_BE="${CALLEE_HASH#0x}"
CALLEE_HASH_LE="$(reverse_hex_bytes "$CALLEE_HASH_BE")"
...
# Ensure the manifest does not require wildcard contract permissions.
if jq -e '.permissions[] | select(.contract == "*")' Caller.manifest.json >/dev/null; then
  echo "error: expected Caller manifest to avoid wildcard contract permissions" >&2
  jq '.permissions' Caller.manifest.json >&2
  exit 1
fi
...
echo "✅ neoxp external member-call smoke test passed"
