...
impl SemanticAnalyzer {
    ...
    fn check_function_signatures(&self, ast: &AstNode, warnings: &mut Vec<String>) {
        self.visit_node(ast, &mut |node, _depth| {
            if let AstNodeType::Function { name, params, returns, .. } = &node.node_type {
                if params.is_empty() && returns.is_empty() {
                    warnings.push(format!("Function '{}' has no parameters or return values", name));
                }
    ...
    fn check_optimization_opportunities(&self, ast: &AstNode, opportunities: &mut Vec<String>) {
        ...
        self.visit_node(ast, &mut |node, _depth| {
            if let AstNodeType::FunctionCall { name, arguments } = &node.node_type {
                if arguments.iter().all(|arg| matches!(arg.node_type, AstNodeType::Literal { .. })) {
                    ...
                    *constant_expressions.entry(expr_key.clone()).or_insert(0) += 1;
                    ...
                    if constant_expressions[&expr_key] > 1 {
                        opportunities.push(format!("Constant expression '{}' appears multiple times", expr_key));
                    }
                }
            }
        });
    ...
    fn is_builtin(&self, name: &str) -> bool {
        matches!(name, 
            "add" | "sub" | "mul" | "div" | "mod" | "eq" | "lt" | "gt" | "iszero" |
            "and" | "or" | "xor" | "not" | "byte" | "shl" | "shr" | "sar" |
            "keccak256" | "sha256" | "ripemd160" | "ecrecover" |
            "mload" | "mstore" | "sload" | "sstore" |
            "caller" | "callvalue" | "calldataload" | "calldatasize" |
            "gas" | "gasprice" | "origin" | "address" | "balance" |
            "timestamp" | "number" | "blockhash" | "coinbase" |
            "log0" | "log1" | "log2" | "log3" | "log4" |
            "call" | "delegatecall" | "staticcall" | "return" | "revert"
        )
    }
}
