...
//! builtin calls into NeoVM instructions. Handles:
//!
//! * **Static slots** — 32-byte BE-padded slots for static types (integers,
//!   addresses, bools, bytesN)
//! * **Dynamic tails** — Offset+length+data layout for strings, bytes,
//!   dynamic arrays, and structs
//! * **Head+tail encoding** — EVM-canonical separated layout for mixed
//!   static/dynamic argument lists
//! * **Struct flattening** — `abi.encode(MyStruct)` expands to per-field
//!   slots (Task #124)
//!
//! ### Related
//!
//! * `abi_decode.rs` — inverse operation
//! * `stdlib.rs` (runtime) — the `invoke_native_stdlib` handler that
//!   produces the head+tail bytes at runtime
