...
use crate::cli::{
    compile_contracts, compile_contracts_with_options, CompileError, CompileOptions,
};
...
fn first_error_code(err: &CompileError) -> Option<ErrorCode> {
    let diags: Option<&[Diagnostic]> = match err {
        ...
        CompileError::Manifest(d) => return Some(d.code.clone()),
        CompileError::Io { .. } | CompileError::Message(_) => None,
    };
    diags.and_then(|d| d.first()).map(|d| d.code.clone())
}
...
fn parse_error_carries_nsh1000() {
    ...
    assert!(
        matches!(code, ErrorCode::Nsh1000 | ErrorCode::Nsh1001 | ErrorCode::Nsh1002 | ErrorCode::Nsh1003 | ErrorCode::Nsh1004 | ErrorCode::Nsh1005),
...
fn semantic_duplicate_signature_carries_nsh2000() {
    ...
    let code = first_error_code(&err).expect("expected a diagnostic code");
    assert_eq!(code, ErrorCode::Nsh2000, "duplicate signature should be NSH-2000");
}
...
fn ir_invalid_storage_param_carries_nsh3000() {
    ...
    let code = first_error_code(&err).expect("expected a diagnostic code");
    assert!(
        matches!(code, ErrorCode::Nsh3000 | ErrorCode::Nsh3001 | ErrorCode::Nsh3002 | ErrorCode::Nsh3003 | ErrorCode::Nsh3004 | ErrorCode::Nsh3005),
...
fn manifest_wildcard_permission_carries_nsh6000() {
    ...
    let err = compile_contracts_with_options(source, false, opts)
        .expect_err("expected manifest error");
    let code = first_error_code(&err).expect("expected a diagnostic code");
    assert!(
        matches!(code, ErrorCode::Nsh6000 | ErrorCode::Nsh6001 | ErrorCode::Nsh6002),
