...
impl Parser {
    ...
    fn parse_if(&mut self) -> Result<Option<AstNode>, CompilerError> {
        ...
        let else_branch = if self.check(&TokenType::Identifier) && 
        ...
        Ok(Some(AstNode {
            node_type: AstNodeType::If { condition, then_branch, else_branch },
            line: start_line,
            column: start_column,
        }))
    }
    ...
    fn parse_for(&mut self) -> Result<Option<AstNode>, CompilerError> {
        ...
        // Parse body
        let body = Box::new(self.parse_statement()?.ok_or_else(|| {
            CompilerError::ParseError("Expected for loop body".to_string())
        })?);
        
        Ok(Some(AstNode {
            node_type: AstNodeType::For { init, condition, update, body },
            line: start_line,
            column: start_column,
        }))
    }
    ...
    fn parse_switch(&mut self) -> Result<Option<AstNode>, CompilerError> {
        ...
        while self.check(&TokenType::Case) || self.check(&TokenType::Default) {
            if self.check(&TokenType::Case) {
                ...
                let body = self.parse_statement()?.ok_or_else(|| {
                    CompilerError::ParseError("Expected case body".to_string())
                })?;
        ...
        Ok(Some(AstNode {
            node_type: AstNodeType::Switch { expression, cases, default },
            line: start_line,
            column: start_column,
        }))
    }
    ...
    fn parse_function(&mut self) -> Result<Option<AstNode>, CompilerError> {
        ...
        let body = Box::new(self.parse_statement()?.ok_or_else(|| {
            CompilerError::ParseError("Expected function body".to_string())
        })?);
        
        Ok(Some(AstNode {
            node_type: AstNodeType::Function { name, params, returns, body },
            line: start_line,
            column: start_column,
        }))
    }
